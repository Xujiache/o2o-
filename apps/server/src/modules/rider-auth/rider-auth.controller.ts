...
import { RIDER_AUTH_IDEMPOTENT_SCOPE } from './rider-auth.constants';
import { LoginByMobileDto, LoginByMobileVo, LogoutVo, RefreshTokenDto, RefreshTokenVo } from './rider-auth.dto';
...
export class RiderAuthController {
  constructor(
    private readonly service: RiderAuthService,
  ...
  @ApiOperation({ summary: '骑手端发送短信验证码(复用 sms 模块)' })
  ...
  @Post('login')
  @Public()
  @Idempotent({ scope: RIDER_AUTH_IDEMPOTENT_SCOPE.login, ttlSeconds: 60 })
  @Audit({ targetType: 'rider-auth' })
  @ApiOperation({ summary: '骑手手机号验证码登录(自动注册)' })
  ...
  @Post('refresh')
  @Public()
  @Idempotent({ scope: RIDER_AUTH_IDEMPOTENT_SCOPE.refresh, ttlSeconds: 60 })
  @ApiOperation({ summary: '刷新骑手 access token(轮换 refresh)' })
  ...
  @Post('logout')
  @UseGuards(RiderJwtGuard)
  @ApiBearerAuth('Rider-Token')
  @Idempotent({ scope: RIDER_AUTH_IDEMPOTENT_SCOPE.logout, ttlSeconds: 60 })
  @Audit({ targetType: 'rider-auth' })
  @ApiOperation({ summary: '骑手登出 — 写 jti 黑名单 + 删除 refresh' })
